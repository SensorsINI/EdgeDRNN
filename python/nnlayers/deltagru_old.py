...
class DeltaGRU(nn.Module):
    ...
    def forward(self, input: Tensor, state: Optional[Tuple] = None, quantize: int = 0, feature_lengths=None):
        ...
        if state is None:  # state = (x_p, h_0, h_p, ma_0, mb_0)
            state = self.initialze_state(x)
        else:
            new_state = []
            for l in range(self.num_layers):
                new_state.append((state[0][l], state[1][l], state[2][l], state[3][l], state[4][l]))
    ...
    def initialze_state(self, x):
        # Initializers
        self.init_input = torch.zeros(self.batch_size, self.x_p_size, dtype=x.dtype, device=x.device)
        self.init_state = torch.zeros(self.batch_size, self.hidden_size, dtype=x.dtype, device=x.device)
        self.init_gates = torch.zeros(self.batch_size, self.gate_size, dtype=x.dtype, device=x.device)
        state = []
        for l in range(self.num_layers):
            bias_ih = getattr(self, 'bias_ih_l{}'.format(l))
            bias_hh = getattr(self, 'bias_hh_l{}'.format(l))
